...
class UpSampleConv2D(torch.jit.ScriptModule):
    def __init__(
        ...
        self.pix = nn.PixelShuffle(self.upscale_factor)
...
class DownSampleConv2D(torch.jit.ScriptModule):
    def __init__(
        ...
        self.down =  nn.PixelUnshuffle(self.downscale_ratio)
    ...
    def forward(self, x):
        ...
        x = self.down(x)
        B,C,W,H = x.shape
        C = int(C // (self.downscale_ratio**2))
        x = x.view(B,int(self.downscale_ratio**2),C,H,W)
        x = x.mean(dim=1)
        x = self.conv(x)
...
class ResBlockDown(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.residual = DownSampleConv2D(input_channels=input_channels,n_filters=n_filters, kernel_size=1)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Generator(torch.jit.ScriptModule):
    ...
    def __init__(self, starting_image_size=4):
        ...
        # Dense layer to project latent z to initial image tensor
    ...
    def forward_given_samples(self, z):
        ...
        x = self.dense(z.to("cuda:0"))
        x = x.view(z.size(0),128,self.starting_image_size,self.starting_image_size)
        x = self.layers(x)
        return x
    ...
    def forward(self, n_samples: int = 1024):
        ...
        # network.
        ##################################################################
        z = torch.randn(n_samples, 128,device="cuda:0")
...
class Discriminator(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        batch_size = x.size(0)
        x = self.layers(x)
        features = x.view(batch_size, 128, -1).sum(dim=2)
        res = self.dense(features)
        return res
